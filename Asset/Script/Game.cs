...
class RandomWeight<T>
{
    ...
    public void ToAdd (T item, double weight)
    {
...
public class Game : MonoBehaviour
{
    ...
    public double priorPropability = 2;
    public int gx;
    public int gy;
    ...
    public void CheckInputGrid()
    {
        ...
        if (Input.GetButtonDown("Fire1"))
        {
            ...
            String colorD = dispColor(Distance);
            ...
            if (tile.iscovered == true)
            {
                    ...
                    {
                        {
                            ...
                            float proba = float.Parse(grid[xx, yy].probability.text);
                            String color = dispColor(Distance);
                            if (NumberOfClicks == 1) {
                                proba = 0.006f;
                            }
                            grid[xx, yy].probability.text =Math.Round( proba *JointTableProbability(color,Distance),4).ToString();
                ...
                for (int yy = 0; yy < 8; yy++)
                {
                    for (int xx = 0; xx < 20; xx++)
                    {
                        {
                            float proba = float.Parse(grid[xx, yy].probability.text);
                            grid[xx, yy].probability.text = Math.Round( proba / sum, 4).ToString();
                        }
                    }
                }
    ...
    public String dispColor (int DistanceFromGhost)
    {
        RandomWeight<String> colorProb = new RandomWeight<String>();

        if (DistanceFromGhost == 3 || DistanceFromGhost == 4)
        {

            colorProb.ToAdd("yellow", 0.5);
            colorProb.ToAdd("orange", 0.150);
            colorProb.ToAdd("red", 0.050);
            colorProb.ToAdd("green", 0.300);

            return colorProb.GetRandom();
        }
        else if (DistanceFromGhost == 1 || DistanceFromGhost == 2)
        {
            colorProb.ToAdd("yellow", 0.150);
            colorProb.ToAdd("orange", 0.500);
            colorProb.ToAdd("red", 0.3);
            colorProb.ToAdd("green", 0.050);

            return colorProb.GetRandom();
        }
        else if (DistanceFromGhost == 0)
        {
            colorProb.ToAdd("yellow", 0.10);
            colorProb.ToAdd("orange", 0.200);
            colorProb.ToAdd("red", 0.6500);
            colorProb.ToAdd("green", 0.050);
        ...
        {
            colorProb.ToAdd("yellow", 0.250);
            colorProb.ToAdd("orange", 0.100);
            colorProb.ToAdd("red", 0.050);
            colorProb.ToAdd("green", 0.600);
    ...
    public float JointTableProbability(string color, int DistanceFromGhost)//Probability tables
    {
    ...
    public void GhostPosition()
    {
        ...
        if (grid[x, y] == null)
        {
            ...
            grid[x, y] = ghostTile;
            grid[x, y].probability.text = "0.006";
            gx = x;
            gy = y;
            Debug.Log("(" + gx + ", " + gy + ")");
            ColorPosition (x, y);
    ...
    public void ColorPosition(int X, int Y)
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            for (int x = 0; x < 20; x++)
            {
                ...
                if (grid[x, y] == null)
                {
                    Tile colorTile =Instantiate(Resources.Load(path, typeof (Tile)),new Vector3(x, y, 0),Quaternion.identity) as Tile;
                    grid[x, y] = colorTile;
                    grid[x, y].probability.text = "0.006";
                    NumberOfTiles++;
                }
